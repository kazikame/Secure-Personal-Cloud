...
def encryptFiles(algorithm, keyFile, toBeUploaded, tempDirectory):
    ...
    for (root, dirnames, filenames) in walk(toBeUploaded):
...
def changePass(keyFile,oldpass,newpass): #input argument one is path to keyFile, rest is self-explanatory. The given file is changed.
